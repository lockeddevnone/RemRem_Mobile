...
// ++++Handico upgrade
            val channelId = "HDCredit"
            val channelName = "HDCredit Service"
            val channel = NotificationChannel(
                channelId,
                channelName, NotificationManager.IMPORTANCE_NONE
            ).apply {
                description = "HDCredit Service Channel"
            }
...
// ++++Handico upgrade
    ...
    //    .setContentTitle("Kết nối xác thực")
...
// ++++Handico upgrade
    private fun updateSchedule() {
        ...
        if (idLogin != -1 && token != null) {
            Log.d("DATA_TEST: ", "idLogin $idLogin")
            updateState(idLogin, token)
        }
    }

    var mStatusChecker: Runnable = Runnable {
        Timer("updateState", false).schedule(0, 300000) {
            updateSchedule()
        }
    }

    @SuppressLint("MissingPermission", "NewApi")
    private fun updateState(id: Int, token: String) {
        try {
            if (isInternetOn()) {
                // Internet Available
